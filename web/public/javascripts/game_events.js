...
document.addEventListener("DOMContentLoaded", event => {
    ...
    let buildGraphData = gameData => {
        ...
        let hideBots = sessions.some(s => !s.session.is_bot && s.session.team != 'spectator')
                ...
                options: {
                    ...
                    elements: {
                        point: {
                            ...
                            pointStyle: (context) => {
                                const pointIcons = {
                                    'team':        (e) => Common.createEl('IMG', { src: e.team != 'spectators' ? '/images/timeline/jointeam-generic.svg' : '/images/timeline/leaveteam-generic.svg', width: '24px', height: '24px' }),
